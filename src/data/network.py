...
class HealthNetwork:
    ...
    def __init__(self, graph: nx.DiGraph = None):
        ...
        self._node_attrs: Dict[int, NodeAttributes] = {}
        self._edge_attrs: Dict[Tuple[int, int], EdgeAttributes] = {}

    def add_node(self, node_id: int, **attributes):
        """Add node with health-specific attributes."""
        self.graph.add_node(node_id, **attributes)
        self._node_attrs[node_id] = NodeAttributes(node_id=node_id, **attributes)
    ...
    def get_edge_probability(self, source: int, target: int) -> float:
        ...
        return self._edge_attrs.get(
            (source, target), EdgeAttributes(source, target)
        ).influence_prob
