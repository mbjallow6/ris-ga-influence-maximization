...
class RRSetGenerator:
    ...
    def __init__(
        ...
        logger.info(
            f"RRSetGenerator initialized: {self.num_nodes} nodes, "
    ...
    def generate_rr_sets(self, theta: int) -> List[RRSet]:
        ...
        """
        logger.info(
            f"Generating {theta} RR sets using {self.parallel_workers} workers..."
        )
        ...
        if self.parallel_workers == 1:
            ...
            with ProcessPoolExecutor(max_workers=self.parallel_workers) as executor:
                if self.diffusion_model == "IC":
                    futures = {
                        executor.submit(self._generate_single_rr_set_ic, root, seed): (
                            root,
                            seed,
                        )
                    ...
                    futures = {
                        executor.submit(self._generate_single_rr_set_lt, root, seed): (
                            root,
                            seed,
                        )
                        for root, seed in tasks
    ...
    def estimate_influence_spread(
        self, seed_set: Set[int], rr_sets: List[RRSet]
    ) -> float:
        """
    ...
    def get_statistics(self, rr_sets: List[RRSet]) -> Dict[str, float]:
        ...
        return {
            ...
            "total_nodes_covered": len(
                set().union(*[rr_set.nodes for rr_set in rr_sets])
            ),
        }
